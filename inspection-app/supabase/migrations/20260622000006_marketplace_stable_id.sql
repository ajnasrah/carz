-- ============================================
-- Stable listing id + ready-to-sell detail.
-- Previously ready-to-sell cars got a random gen_random_uuid() id each call, so
-- "view details" broke on reload (the id didn't exist). Key every listing by a
-- DETERMINISTIC id derived from stock_number, and make the detail RPC resolve it
-- (works for ready-to-sell-only cars, not just inspected ones).
-- ============================================
...
BEGIN
  ...
  SELECT * FROM (
    SELECT DISTINCT ON (inv.stock_number)
      md5(inv.stock_number)::uuid                    AS id,
      inv.stock_number,
...
-- ---- DETAIL ----  (now keyed by the same md5(stock_number) id; works for
-- ready-to-sell-only cars, and merges ready-to-sell photos into checklist.photos)
...
RETURNS TABLE (
  ...
  stock_number text, total_cost text, days_on_lot text, sa_status text
)
LANGUAGE sql STABLE SECURITY DEFINER AS $$
  SELECT DISTINCT ON (inv.stock_number)
    md5(inv.stock_number)::uuid,
    COALESCE(i.vin, inv.last_6_vin),
    inv.last_6_vin,
    inv.vehicle_vin,
    COALESCE(i.year::text, inv.vehicle_year),
    COALESCE(i.make, inv.vehicle_make),
    COALESCE(i.model, inv.vehicle_model),
