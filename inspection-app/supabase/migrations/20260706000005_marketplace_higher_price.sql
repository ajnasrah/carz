-- Marketplace price = higher of Buy Now / Opening Price.
-- Recreates the two listing RPCs from 20260706000004 with the displayed price
-- switched from sac.buy_now to GREATEST(sac.buy_now, sac.opening_price). GREATEST
-- ignores NULLs, so: both present → higher of the two; only one present → that one;
-- both NULL → NULL. Cars with only an opening price now show a number instead of
-- blank. (The mark_sold / unmark_sold funcs below are unchanged re-declarations.)
-- Coverage + sold-hide behaviour from ...000004 is preserved verbatim otherwise.
...
BEGIN
  ...
  WITH inv_listings AS (
    SELECT DISTINCT ON (inv.stock_number)
      ...
      GREATEST(sac.buy_now, sac.opening_price)::text AS buy_now, sac.detail_url AS sa_url
    FROM inventory inv
...
LANGUAGE sql STABLE SECURITY DEFINER AS $$
  ...
  (SELECT DISTINCT ON (inv.stock_number)
    ...
    GREATEST(sac.buy_now, sac.opening_price)::text, sac.detail_url
  FROM inventory inv
  LEFT JOIN inspections i ON (
    (i.vin_last6 = inv.last_6_vin OR i.vin = inv.last_6_vin OR i.vin = inv.vehicle_vin
     OR (i.stock_number IS NOT NULL AND i.stock_number = inv.stock_number)) AND i.completed_at IS NOT NULL)
  ...
  (SELECT
    ...
    NULL::text, NULL::text, NULL::text, NULL::text, GREATEST(sac.buy_now, sac.opening_price)::text, sac.detail_url
  FROM sa_active_cars sac
