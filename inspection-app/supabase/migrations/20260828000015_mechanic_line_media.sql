-- Carry the inspector's photo and voice note through to the tech's line.
--
-- A voice note nobody can play is worse than no voice note: the inspector
-- believes he passed the information on and the mechanic never receives it. A
-- noise is also the one finding that genuinely does not survive being written
-- down — "whine from the rear" narrows it to about six repairs, and the ten
-- seconds of audio narrows it to one.
--
-- Media travels BY REFERENCE. The files live once in inspection-photos where
-- the inspection put them; the line stores the url/path pairs, the same way the
-- body shop reads a car's photos rather than owning copies of them.

ALTER TABLE mechanic_lines
  ADD COLUMN IF NOT EXISTS media JSONB NOT NULL DEFAULT '[]'::jsonb;

COMMENT ON COLUMN mechanic_lines.media IS
'Photos and voice notes the inspector attached to the finding that raised this
line: [{kind:"photo"|"audio", url, path}]. References into inspection-photos —
never copies.';
...
BEGIN
  ...
  -- 'sa_' keys are the LISTING flow's disclosure rows, written long after
  -- arrival — routing them would reopen repairs already paid for.
  ...
  CREATE TEMP TABLE IF NOT EXISTS _wo_findings (
    source_key TEXT, descr TEXT, sys TEXT, sev TEXT, media JSONB
  ) ON COMMIT DROP;
  DELETE FROM _wo_findings;

  INSERT INTO _wo_findings (source_key, descr, sys, sev, media)
  WITH secs AS (
  ...
  raw AS (
    ...
    -- Legacy: a failed check with one note and no findings list.
    SELECT
      s.sec || ':' || s.check_id,
  ...
  SELECT
    ...
    -- Photos and voice notes flattened into one list the line can render.
    coalesce((
      SELECT jsonb_agg(m) FROM (
        SELECT jsonb_build_object('kind','photo','url',p ->> 'url','path',p ->> 'path') AS m
        FROM jsonb_array_elements(coalesce(finding -> 'photos', '[]'::jsonb)) p
        WHERE p ->> 'url' IS NOT NULL
        UNION ALL
        SELECT jsonb_build_object('kind','audio','url',a ->> 'url','path',a ->> 'path')
        FROM jsonb_array_elements(coalesce(finding -> 'audio', '[]'::jsonb)) a
  ...
  IF v_mech_job IS NOT NULL THEN
    FOR v_rec IN SELECT * FROM _wo_findings LOOP
      INSERT INTO mechanic_lines
        (job_id, system, description, severity, status, source_inspection_id, source_key, media)
      VALUES
        (v_mech_job, v_rec.sys, left(v_rec.descr, 500), v_rec.sev, 'open',
         p_inspection_id, v_rec.source_key, v_rec.media)
      ...
      DO UPDATE SET
        ...
        updated_at  = NOW();
        -- Status and severity stay as the mechanic left them.
    END LOOP;
  END IF;
